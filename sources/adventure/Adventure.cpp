...
{
    if (name == "linear") {
        outInterpolation = PropMoveInterpolation::Linear;
        return true;
    }
    if (name == "accelerate") {
        outInterpolation = PropMoveInterpolation::Accelerate;
        return true;
    }
    if (name == "decelerate") {
        outInterpolation = PropMoveInterpolation::Decelerate;
        return true;
    }
    if (name == "accelerateDecelerate") {
        outInterpolation = PropMoveInterpolation::AccelerateDecelerate;
        return true;
    }
    if (name == "overshoot") {
        outInterpolation = PropMoveInterpolation::Overshoot;
        return true;
    }
    return false;
}
...
float AdventureApplyPropMoveInterpolation(PropMoveInterpolation interpolation, float t)
{
    ...
    switch (interpolation) {
        case PropMoveInterpolation::Accelerate:
            return t * t;

        case PropMoveInterpolation::Decelerate:
            return 1.0f - (1.0f - t) * (1.0f - t);

        case PropMoveInterpolation::AccelerateDecelerate:
            return t * t * (3.0f - 2.0f * t);

        case PropMoveInterpolation::Overshoot:
        {
        ...
        case PropMoveInterpolation::Linear:
        default:
            return t;
    }
}
