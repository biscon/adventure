        ...
        ResourceScope scope)
{
    ...
    auto existing = resources.spriteAssetHandleByPath.find(sidecarNorm);
    ...
    json aseprite;
    {
        std::ifstream in(asepriteJsonPath.string());
        if (!in.is_open()) {
            TraceLog(LOG_ERROR, "Failed to open Aseprite JSON: %s", asepriteJsonPath.string().c_str());
            return -1;
        }
        in >> aseprite;
    }
    ...
    asset.sidecarPath = sidecarNorm;
    asset.asepriteJsonPath = asepriteJsonPath.string();
    asset.baseDrawScale = sidecar.value("baseDrawScale", 1.0f);
    asset.useRightForLeft = sidecar.value("useRightForLeft", true);
    asset.scope = scope;

    if (sidecar.contains("feetPivot")) {
        const auto& feet = sidecar["feetPivot"];
        asset.feetPivot.x = feet.value("x", 0.0f);
        asset.feetPivot.y = feet.value("y", 0.0f);
    }
    ...
    if (imageRel.empty()) {
        TraceLog(LOG_ERROR, "Aseprite JSON missing meta.image: %s", asepriteJsonPath.string().c_str());
        return -1;
    }
    ...
    resources.spriteAssetHandleByPath[sidecarNorm] = asset.handle;

    TraceLog(LOG_INFO, "Loaded sprite asset: %s", sidecarNorm.c_str());
    return asset.handle;
}
