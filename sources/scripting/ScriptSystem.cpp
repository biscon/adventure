    ...
    switch (co.waitType) {
        ...
        case ScriptWaitType::WalkComplete:
        {
            ...
            if (actorIndex >= 0 &&
                ...
                out << "  " << state.adventure.actors[actorIndex].actorId;
            } else {
                out << "  <invalid actor>";
            }
            break;
...
void ScriptSystemUpdate(GameState& state, float dt)
{
    ...
    for (ScriptCoroutine& co : script.coroutines) {
        ...
        switch (co.waitType) {
            ...
            case ScriptWaitType::SpeechComplete:
                shouldResume = !state.adventure.speechUi.active;
                break;
            ...
            case ScriptWaitType::DialogueChoice:
                if (!dialogueResultConsumed && state.adventure.dialogueUi.resultReady) {
                    co.hasPendingResumeString = true;
                    co.pendingResumeString = state.adventure.dialogueUi.selectedOptionId;
                    shouldResume = true;
