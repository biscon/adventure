...
static std::vector<TriangulationPolygon> BuildFinalWalkablePolygons(const NavMeshData& navMesh)
{
    Paths64 walkPaths;
    ...
    Paths64 mergedWalk = Union(walkPaths, FillRule::NonZero);
    ...
    if (!blockerPaths.empty()) {
        finalPaths = Difference(mergedWalk, blockerPaths, FillRule::NonZero);
    }
...
bool BuildNavMesh(NavMeshData& navMesh)
{
    ...
    const std::vector<TriangulationPolygon> finalPolys = BuildFinalWalkablePolygons(navMesh);
